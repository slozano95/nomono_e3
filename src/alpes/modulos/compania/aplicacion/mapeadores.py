...
from alpes.modulos.vuelos.dominio.entidades import Compania, Reserva, Aeropuerto
from alpes.modulos.vuelos.dominio.objetos_valor import Itinerario, Odo, Segmento, Leg
from alpes.seedwork.aplicacion.dto import DTO
from alpes.modulos.vuelos.aplicacion.dto import ReservaDTO
...
class MapeadorCreacion(AppMap):
    def externo_a_dto(self, externo: dict) -> CreacionCompaniaDTO:
        dto = CreacionCompaniaDTO(externo)
        return dto
...
class MapeadorCreacionRepo(RepMap):
    ...
    def dto_a_entidad(self, dto: CompaniaModel) -> Compania:
        data = Compania()
        data.id = dto.id
